...
const PROJECT_STATUSES = ["COMPLETED", "IN_PROGRESS", "JUST_STARTED"] as const;

function parseProjectStatus(value?: string) {
  if (value && PROJECT_STATUSES.includes(value as (typeof PROJECT_STATUSES)[number])) {
    return value as (typeof PROJECT_STATUSES)[number];
  }
  return "JUST_STARTED";
}

async function ensureProfile(userId: string, fallbackName = "Your Name") {
...
function owner(req: AuthedRequest) {
  return req.session!.sub;
}

portfolioRouter.use(requireAuth);

portfolioRouter.get("/me", async (req: AuthedRequest, res) => {
...
portfolioRouter.post("/onboarding/complete", async (req: AuthedRequest, res) => {
...
portfolioRouter.put("/profile", async (req: AuthedRequest, res) => {
...
portfolioRouter.get("/projects", async (req: AuthedRequest, res) => {
  const data = await prisma.project.findMany({ where: { userId: owner(req) }, orderBy: { sortOrder: "asc" } });
  res.json({ ok: true, items: data });
});
portfolioRouter.post("/projects", async (req: AuthedRequest, res) => {
...
portfolioRouter.put("/projects/:id", async (req: AuthedRequest, res) => {
  const params = idParams.safeParse(req.params);
  const parsed = projectSchema.safeParse(req.body);
  if (!params.success || !parsed.success) return res.status(400).json({ error: "Invalid payload" });
...
portfolioRouter.delete("/projects/:id", async (req: AuthedRequest, res) => {
  const params = idParams.safeParse(req.params);
...
portfolioRouter.get("/skills", async (req: AuthedRequest, res) => {
  const data = await prisma.skill.findMany({ where: { userId: owner(req) }, orderBy: { sortOrder: "asc" } });
  res.json({ ok: true, items: data });
});
portfolioRouter.post("/skills", async (req: AuthedRequest, res) => {
...
portfolioRouter.put("/skills/:id", async (req: AuthedRequest, res) => {
  const params = idParams.safeParse(req.params);
  const parsed = skillSchema.safeParse(req.body);
  if (!params.success || !parsed.success) return res.status(400).json({ error: "Invalid payload" });
...
portfolioRouter.delete("/skills/:id", async (req: AuthedRequest, res) => {
  const params = idParams.safeParse(req.params);
  if (!params.success) return res.status(400).json({ error: "Invalid id" });
...
portfolioRouter.get("/awards", async (req: AuthedRequest, res) => {
  const data = await prisma.award.findMany({ where: { userId: owner(req) }, orderBy: { sortOrder: "asc" } });
  res.json({ ok: true, items: data });
});
portfolioRouter.post("/awards", async (req: AuthedRequest, res) => {
  const parsed = awardSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: "Invalid award payload" });
  const created = await prisma.award.create({ data: { userId: owner(req), ...parsed.data } });
  res.status(201).json({ ok: true, item: created });
});
portfolioRouter.put("/awards/:id", async (req: AuthedRequest, res) => {
  const params = idParams.safeParse(req.params);
  const parsed = awardSchema.safeParse(req.body);
  if (!params.success || !parsed.success) return res.status(400).json({ error: "Invalid payload" });
...
portfolioRouter.delete("/awards/:id", async (req: AuthedRequest, res) => {
  const params = idParams.safeParse(req.params);
  if (!params.success) return res.status(400).json({ error: "Invalid id" });
...
portfolioRouter.get("/leadership", async (req: AuthedRequest, res) => {
  const data = await prisma.leadership.findMany({ where: { userId: owner(req) }, orderBy: { sortOrder: "asc" } });
  res.json({ ok: true, items: data });
});
portfolioRouter.post("/leadership", async (req: AuthedRequest, res) => {
  const parsed = leadershipSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: "Invalid leadership payload" });
  const created = await prisma.leadership.create({ data: { userId: owner(req), ...parsed.data } });
  res.status(201).json({ ok: true, item: created });
});
portfolioRouter.put("/leadership/:id", async (req: AuthedRequest, res) => {
  const params = idParams.safeParse(req.params);
  const parsed = leadershipSchema.safeParse(req.body);
  if (!params.success || !parsed.success) return res.status(400).json({ error: "Invalid payload" });
...
portfolioRouter.delete("/leadership/:id", async (req: AuthedRequest, res) => {
  const params = idParams.safeParse(req.params);
  if (!params.success) return res.status(400).json({ error: "Invalid id" });
...
portfolioRouter.get("/experience", async (req: AuthedRequest, res) => {
  const data = await prisma.experience.findMany({ where: { userId: owner(req) }, orderBy: { sortOrder: "asc" } });
  res.json({ ok: true, items: data });
});
portfolioRouter.post("/experience", async (req: AuthedRequest, res) => {
  const parsed = experienceSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: "Invalid experience payload" });
  const created = await prisma.experience.create({ data: { userId: owner(req), ...parsed.data } });
  res.status(201).json({ ok: true, item: created });
});
portfolioRouter.put("/experience/:id", async (req: AuthedRequest, res) => {
  const params = idParams.safeParse(req.params);
  const parsed = experienceSchema.safeParse(req.body);
  if (!params.success || !parsed.success) return res.status(400).json({ error: "Invalid payload" });
...
portfolioRouter.delete("/experience/:id", async (req: AuthedRequest, res) => {
  const params = idParams.safeParse(req.params);
  if (!params.success) return res.status(400).json({ error: "Invalid id" });
...
portfolioRouter.get("/education", async (req: AuthedRequest, res) => {
  const data = await prisma.education.findMany({ where: { userId: owner(req) }, orderBy: { sortOrder: "asc" } });
  res.json({ ok: true, items: data });
});
portfolioRouter.post("/education", async (req: AuthedRequest, res) => {
  const parsed = educationSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: "Invalid education payload" });
  const created = await prisma.education.create({ data: { userId: owner(req), ...parsed.data } });
  res.status(201).json({ ok: true, item: created });
});
portfolioRouter.put("/education/:id", async (req: AuthedRequest, res) => {
  const params = idParams.safeParse(req.params);
  const parsed = educationSchema.safeParse(req.body);
  if (!params.success || !parsed.success) return res.status(400).json({ error: "Invalid payload" });
...
portfolioRouter.delete("/education/:id", async (req: AuthedRequest, res) => {
  const params = idParams.safeParse(req.params);
  if (!params.success) return res.status(400).json({ error: "Invalid id" });
